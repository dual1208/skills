...
```bash
npx skills add dual1208/skills --skill=ask-matt
```
...
[Source](https://github.com/dual1208/skills/tree/main/skills/engineering/ask-matt)
...
For design discussions, `/grill-with-docs` asks independent questions in rounds, waits for the answers, and updates agreed terms and significant decisions after each answered round. `/improve-codebase-architecture` starts with a review of the actual system, then uses the same discussion and document workflow for the chosen change. Neither restarts choices already settled.
